...
if (figma.editorType === 'figma') {
    ...
    // ーーーーーーーー選択なし　→ msg："not selected"　を返す　ーーーーーーーーーー
    if (figma.currentPage.selection.length == 0) {
        figma.ui.postMessage({ type: "result", body: "not selected" })
    }
    else {
        // ーーーーーーーーーー  実行　close　ーーーーーーーーーー
        if (msg.type === 'close') {
            figma.closePlugin();
        }

        // ーーーーーーーーーー  実行　逆転  ーーーーーーーーーー
        if (msg.type === 'reverse') {
        ...
        // ーーーーーーーーーー  実行　小文字に変換 ーーーーーーーーーー
        if (msg.type === 'small_letter') {
        ...
        // ーーーーーーーーーー  実行　文字数カウント ーーーーーーーーーー
        if (msg.type === 'count') {
        ...
        // ーーーーーーーーーー  実行　px_check  ーーーーーーーーーー
                    ...
                    for (let node of figma.currentPage.selection) {
                     ...
                     // ーーーーーーーー1層目と２層目のフレームの　「重なり」＆「空き」　をチェック
                        if (node.visible != false && node.name != 'SP_ガイド' && node.name != 'PC_ガイド' && node.rotation == 0) {
                            ...
                            // ■■■■■■rect作成関数■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■
                            let makeRect = (child, device = 0, wei = 8.0, x = 0, y = child.absoluteBoundingBox.y ) => {
                                ...
                                for (let A of two_layer_allys) {
                                    if (i[0] == A[0]) {}
                                        ...
                                        abs_bottom_numbers.push(Math.abs(i[2] - A[2]))
                                            ...
                                            if ( A[1] < i[1] && A[2] < i[2]){
                                                // abs_top　主人ID 相手ID　Aと　相手のBの絶対値　　　比べた数どうし　
